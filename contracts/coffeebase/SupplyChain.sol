...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "Address is not matched");
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) {
    require(msg.value >= _price, "Value is not enough");
    _;
  }
  ...
  modifier checkValue(uint _upc) {
    ...
    address payable addressToRefund = _make_payable(items[_upc].consumerID);
    addressToRefund.transfer(amountToReturn);
  }
  ...
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "Item state is not for sale");
    _;
  }
